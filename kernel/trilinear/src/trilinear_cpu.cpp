#include "../include/trilinear_cpu.h"


void TriLinearForwardCpu( const float* lut,
                          const float* image,
                          float* output,
                          const int dim,
                          const int shift,
...
void TriLinearBackwardCpu( const float* image,
...
int trilinear_forward_cpu( torch::Tensor lut,
                       torch::Tensor image,
                       torch::Tensor output,
                       int lut_dim,
                       int shift,
...
int trilinear_backward_cpu( torch::Tensor image,
                          ...
                          const int channels )
{
    ...
    for ( index = 0; index < output_size; ++index )
    {
        ...
        output[index] = w000 * lut[id000] + w100 * lut[id100] + w010 * lut[id010]
                        + w110 * lut[id110] + w001 * lut[id001] + w101 * lut[id101]
                        + w011 * lut[id011] + w111 * lut[id111];

        output[index + width * height]
        = w000 * lut[id000 + shift] + w100 * lut[id100 + shift] + w010 * lut[id010 + shift]
          + w110 * lut[id110 + shift] + w001 * lut[id001 + shift] + w101 * lut[id101 + shift]

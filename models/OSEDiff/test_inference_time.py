...
def parse_args():
    ...
    parser.add_argument("--use_distilled", action="store_true",
                        help="Use distilled DCAESR student (student.py) instead of OSEDiff_inference_time")
    # Tile settings
...
def main():
    ...
    if args.use_distilled:
        from student import DistilledOSEDiff_inference_time as OSEDiff_inference_time
    ...
    # RAM tagger — only need for original OSEDiff (distilled student ignores prompts)
    DAPE = None
    if not args.use_distilled:

...
# When --use_distilled is passed, swap in the lightweight student model.
# Architecture config and weights are auto-detected from the checkpoint.
_DISTILLED_IMPORT_DONE = False
...
if __name__ == "__main__":
    ...
    # distilled student
    parser.add_argument("--use_distilled", action="store_true",
                        help="Use distilled DCAESR student (student.py) instead of OSEDiff_test")
    # tile setting
    ...
    if args.use_distilled:
        from student import DistilledOSEDiff_test as OSEDiff_test
        # Let the distilled wrapper read architecture config from the ckpt
        args.distilled_ckpt_path = args.osediff_path
    else:
        from osediff import OSEDiff_test

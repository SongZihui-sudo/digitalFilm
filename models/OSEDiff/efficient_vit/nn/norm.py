...
class TritonRMSNorm2d(nn.LayerNorm):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        try:
            ...
            # Triton JIT 编译失败时的纯 PyTorch fallback（Windows 常见）
            return _rms_norm_pytorch(x, self.weight, self.bias, self.eps)


def _rms_norm_pytorch(
    x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor, eps: float
) -> torch.Tensor:
    """纯 PyTorch RMSNorm2d — 与 TritonRMSNorm2d 语义一致。"""
    x_float = x.float()
    rrms = torch.rsqrt(x_float.pow(2).mean(dim=1, keepdim=True) + eps)
    out = x_float * rrms
...
def reset_bn(
    ...
    from .dcae.apps.utils import AverageMeter, is_master, sync_tensor
    from .dcae.models.utils import get_device, list_join

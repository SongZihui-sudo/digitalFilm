...
from ..nn.ops import (
    ...
    ResBlock,
    ResidualBlock,
    StainAwarePixelUnshuffleDownsample,
    StainAwarePixelShuffleUpSample,
)
...
def build_downsample_block(block_type: str, in_channels: int, out_channels: int, shortcut: Optional[str]) -> nn.Module:
    ...
    elif shortcut == "stain_aware":
        shortcut_block = StainAwarePixelUnshuffleDownsample(
            in_channels=in_channels, out_channels=out_channels, factor=2
        )
        block = ResidualBlock(block, shortcut_block)
    else:
...
def build_upsample_block(block_type: str, in_channels: int, out_channels: int, shortcut: Optional[str]) -> nn.Module:
    ...
    if shortcut is None:
        ...
        shortcut_block = StainAwarePixelShuffleUpSample(in_channels=in_channels, out_channels=out_channels, factor=2)
        block = ResidualBlock(block, shortcut_block)
    else:
...
def build_decoder_project_in_block(in_channels: int, out_channels: int, shortcut: Optional[str]):
    ...
    if shortcut is None:
        ...
        shortcut_block = StainAwarePixelShuffleUpSample(in_channels=in_channels, out_channels=out_channels, factor=1)
        block = ResidualBlock(block, shortcut_block)
    else:
...
class DCAE(nn.Module):
    def __init__(self, cfg: DCAEConfig):
        super().__init__()
        self.cfg = cfg
        self.encoder = DCAE_Encoder(cfg.encoder)
        self.decoder = DCAE_Decoder(cfg.decoder)

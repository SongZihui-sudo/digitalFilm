...
class Camera(Renderer):
    ...
    def __init__(
        ...
        if sensor_type == "simple":
            from models.end2end_imaging.sensor import Sensor

            self.sensor = Sensor.from_config(sensor_file)
        elif sensor_type == "rgb":
            from models.end2end_imaging.sensor import RGBSensor

            self.sensor = RGBSensor.from_config(sensor_file)
        elif sensor_type == "mono":
            from models.end2end_imaging.sensor import MonoSensor
        ...
        if lens_type == "geolens":
            from models.end2end_imaging.deeplens import GeoLens

            self.lens = GeoLens(lens_file, device=device)
        elif lens_type == "hybridlens":
            from models.end2end_imaging.deeplens import HybridLens
            ...
            from models.end2end_imaging.deeplens import DiffractiveLens

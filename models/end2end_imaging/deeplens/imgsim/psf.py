...
def conv_psf_occlusion(img, depth, psf_kernels, psf_depths):
    ...
    # Soft allocation: each pixel contributes to its nearest 2–3 depth layers
    # with Gaussian weights in disparity space (linear in defocus amount).
    depth_expanded = depth.view(B, 1, H, W).expand(B, num_layers, H, W)
    psf_depths_view = psf_depths.view(1, num_layers, 1, 1)

    # Disparity (1/depth) gives uniformly spaced CoC levels
    disp = 1.0 / depth_expanded.abs()
    disp_ref = 1.0 / psf_depths_view.abs()
    layer_spacing = (disp_ref[0, 1, 0, 0] - disp_ref[0, 0, 0, 0]).abs()

    dist_sq = ((disp - disp_ref) / (layer_spacing + 1e-8)) ** 2  # [B, L, H, W]
    weights = F.softmax(-dist_sq / 0.3, dim=1)  # [B, L, H, W] — narrow softmax for minimal bleeding
    ...
    for i in range(num_layers):
        soft_mask = weights[:, i:i+1, :, :]  # [B, 1, H, W]

        if soft_mask.max() < 1e-6:
            continue

        layer_rgb = img * soft_mask  # [B, C, H, W]
        ...
        mask_pad = F.pad(soft_mask, (pad_left, pad_right, pad_top, pad_bottom), mode="constant", value=0)

...
from .digitalFilm_g import FilmPipeline
from utils.utils import compile_model


class digitalFilmv2(model_base):
    """Top-level model wrapping the physical film pipeline + discriminator."""
    ...
    def __init__(self, rank, global_config: dict, config: dict) -> None:
        ...
        # physical film emulation generator
        self.g: FilmPipeline = FilmPipeline(config)
        self.d: patchDiscriminator = patchDiscriminator(
            3, config.ndf, config.disc_n_layers, torch.nn.InstanceNorm2d,
            config.disc_no_antialias, config.disc_patch_size
        )
